...
import { ChevronLeft, Heart, Share2, ShoppingCart, Zap, Award, Clock } from 'lucide-react';
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-ivory dark:bg-dark-bg">
        <motion.div className="text-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          <h1 className="text-4xl font-playfair font-bold mb-6 text-deep-brown dark:text-cream">Produit non trouvé</h1>
          <p className="text-warm-gray dark:text-dark-muted mb-8">Désolé, ce produit n'existe pas.</p>
          <Link href="/products" className="px-8 py-4 bg-copper hover:bg-copper/90 text-white font-semibold rounded-sm transition-colors inline-block">
    ...
    <div className="min-h-screen bg-ivory dark:bg-dark-bg">
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-2 text-sm">
            <Link href="/" className="text-copper hover:text-terracotta flex items-center gap-1">
              <ChevronLeft size={16} />
              Accueil
            </Link>
            <span className="text-clay/40">/</span>
            <Link href="/products" className="text-copper hover:text-terracotta">
              Collection
            </Link>
            <span className="text-clay/40">/</span>
            <span className="text-warm-gray dark:text-dark-muted font-semibold">{product.name}</span>
          </div>
        </div>
      </motion.div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 mb-20">
          ...
          <motion.div variants={fadeInUp} initial="initial" animate="animate">
            ...
            {product.images.length > 1 && (
              <motion.div className="grid grid-cols-4 gap-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}>
          ...
          <motion.div variants={fadeInUp} initial="initial" animate="animate" transition={{ delay: 0.1 }}>
            ...
            <motion.div className="mb-8" variants={fadeInUp}>
              <p className="text-sm text-warm-gray dark:text-dark-muted mb-2 font-semibold">Prix</p>
              <div className="flex items-baseline gap-4">
                <span className="text-5xl font-bold text-copper">{formatPrice(product.price)}</span>
            ...
            {/* Description */}
            <motion.div className="mb-10 pb-10 border-b border-clay/20" variants={fadeInUp}>
              <h3 className="text-sm font-bold uppercase tracking-widest text-deep-brown dark:text-cream mb-4">Description</h3>
            ...
            {/* Features */}
            <motion.div className="grid grid-cols-2 gap-6 mb-12" variants={fadeInUp}>
              <div className="flex gap-4">
                ...
                <div>
                  <p className="font-bold text-deep-brown dark:text-cream mb-1">Artisanat</p>
                  <p className="text-sm text-warm-gray dark:text-dark-muted">Façonné à la main</p>
                </div>
              </div>
              <div className="flex gap-4">
                <div className="w-12 h-12 rounded-lg bg-copper/10 flex items-center justify-center flex-shrink-0">
                ...
                <div>
                  <p className="font-bold text-deep-brown dark:text-cream mb-1">Unique</p>
                  <p className="text-sm text-warm-gray dark:text-dark-muted">Pièce unique</p>
                </div>
              </div>
              <div className="flex gap-4">
                <div className="w-12 h-12 rounded-lg bg-copper/10 flex items-center justify-center flex-shrink-0">
                ...
                <div>
                  <p className="font-bold text-deep-brown dark:text-cream mb-1">Héritage</p>
                  <p className="text-sm text-warm-gray dark:text-dark-muted">Tradition algérienne</p>
                </div>
              </div>
              <div className="flex gap-4">
                <div className="w-12 h-12 rounded-lg bg-copper/10 flex items-center justify-center flex-shrink-0">
                ...
                <div>
                  <p className="font-bold text-deep-brown dark:text-cream mb-1">Qualité</p>
                  <p className="text-sm text-warm-gray dark:text-dark-muted">Premium</p>
                </div>
              </div>
            </motion.div>
            ...
            <motion.div className="flex gap-4 mb-8" variants={fadeInUp}>
              ...
              >
                <Heart
                  size={24}
                  className={isFavorite ? 'fill-red-500 text-red-500' : 'text-copper'}
                />
              </button>
            ...
            <motion.div className="grid grid-cols-2 gap-4" variants={fadeInUp}>
              <div className="bg-cream dark:bg-dark-card p-4 rounded-lg border border-clay/10">
                <p className="text-xs font-bold uppercase tracking-widest text-warm-gray dark:text-dark-muted mb-2">ID Produit</p>
                <p className="font-mono text-sm text-deep-brown dark:text-cream">{product.id}</p>
              </div>
              <div className="bg-cream dark:bg-dark-card p-4 rounded-lg border border-clay/10">
                <p className="text-xs font-bold uppercase tracking-widest text-warm-gray dark:text-dark-muted mb-2">Catégorie</p>
                <p className="font-semibold text-deep-brown dark:text-cream">{product.category || 'Poterie'}</p>
              </div>
            </motion.div>
          </motion.div>
        </div>
        ...
        {relatedProducts.length > 0 && (
          ...
          >
            <div className="mb-12 border-t border-clay/20 pt-12">
              ...
              <motion.div className="grid grid-cols-1 md:grid-cols-3 gap-8" variants={{ staggerChildren: 0.1 }}>
                      ...
                      <div className="bg-white dark:bg-dark-card rounded-lg overflow-hidden border border-clay/10 hover:border-copper/50 transition-all h-full flex flex-col">
                        ...
                        <div className="p-6 flex flex-col flex-grow">
                          ...
                          <div className="pt-4 border-t border-clay/10 flex justify-between items-center">
                            <span className="text-2xl font-bold text-copper">{formatPrice(related.price)}</span>
                            <span className="text-copper font-semibold text-sm">→</span>
                          </div>
                        </div>
                      </div>
                    </Link>
        ...
        <motion.section
          ...
          viewport={{ once: true }}
        >
          <h3 className="text-3xl font-playfair font-bold text-ivory mb-6">Prêt à ajouter cette pièce à votre collection?</h3>
          <p className="text-ivory/80 mb-8 max-w-2xl mx-auto">Chaque pièce est unique et façonnée avec passion. Une fois vendue, elle ne peut pas être reproduite à l'identique.</p>
          <button
            disabled={!product.available}
